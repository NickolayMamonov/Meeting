...
fun MeetingDto.toDomain(): Meeting = Meeting(
    ...
    personHost = personHost?.let { personHostDto ->
        PersonHost(
            id = personHostDto.id,
    ...
    communityHost = communityHost?.let { communityHostDto ->
        CommunityHost(
    ...
    participants = participants.map { personDto ->
        Person(
            id = personDto.id,
            name = personDto.name,
...
fun String.toMeetingStatus(): MeetingStatus = when (uppercase().trim()) {
